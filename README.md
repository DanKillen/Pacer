...
This .NET project ```Pacer``` provides a solution containing separate Data, Test and Web(MVC) projects. When installed and used to create a new project, all references to Pacer will be replaced with the name of your project.
...
1. An implementation of ```Pacer.Data.Services.IUserService``` using EntityFramework to handle data storage/retrieval is provided via ```Pacer.Data.Services.UserServiceDb```.
2. An implementation of ```Pacer.Data.Services.IMailService``` using the .NET Smtp Mail provider to handle email sending is provided via ```Pacer.Data.Services.SmtpMailService```.

Password hashing functionality is added via the ```Pacer.Data.Security.Hasher``` class. This is used in the Data project ```UserServiceDb``` to hash the user password before storing in database.
...
To install this solution as a pacer (pacer name is **termonbase**)

1. Download current version of the pacer

    ```$ git clone https://github.com/termon/DotNetPacerBase.git```

2. Install the pacer so it can be used by ```dotnet new``` command. Use the path (i.e the directory location)to the cloned pacer directory without trailing '/'

    Linux/macOS

    ```$ dotnet new install /path/DotNetPacerBase```

    Windows

    ```c:> dotnet new install c:\path\DotNetPacerBase```

3. Once installed you can create a new project using this pacer.

    ```dotnet new termonbase -n SolutionName```

4. To uninstall a pacer (no longer can be used with dotnet new ).

    ```dotnet new uninstall /path/DotNetPacerBase```
